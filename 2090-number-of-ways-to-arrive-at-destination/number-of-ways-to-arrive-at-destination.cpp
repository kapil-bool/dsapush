...
public:
    ...
    int countPaths(int n, vector<vector<int>>& roads) {
        const int MOD = 1e9 + 7;
         ...
         // crete the adj list and no of ways to reach here so like 1->2->final
         //the no ways i reach 2 will contribute to the final as each represent a unoique path
        ...
        while (!minHeap.empty()) {
            ...
            if (currTime > shortestTime[currNode]) continue;
             //too condtion are here 
             //1-> same disatnce a possible way
             //2-> u found a samllere distance
